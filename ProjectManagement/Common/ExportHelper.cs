...
namespace ProjectManagement
{
    ...
    public class ExcelHelper : IDisposable
    {
        ...
        /// <summary>
        /// 构造函数，新建一个工作簿
        /// updated by zhuguanjun 2017/05/26
        /// </summary>
        public ExcelHelper(string outputFilePath)
        {
            if (string.IsNullOrEmpty(outputFilePath))
                throw new Exception("输出Excel文件路径不能为空！");
            this.outputFile = outputFilePath;
            ...
            for (int i = 0; i < 1; i++)
            {
                ...
                //excelApp.Sheets.Add(missing, missing, 1, Microsoft.Office.Interop.Excel.XlSheetType.xlWorksheet);
            }
        }
        ...
        public void ArrayToExcel(object[,] arr, int top, int left)
        {
            ...
            range = workSheet.Cells[top, left];
        ...
        {
            return (string)((Range)workSheet.Cells[rowIndex, columnIndex]).Text.ToString();

            //for (int i = 1; i <= workSheet.UsedRange.Cells.Rows.Count; i++)
            //{
            //    for (int j = 1; j <= workSheet.UsedRange.Cells.Columns.Count; j++)
            //    {
        ...
        public void InsertRows(int rowIndex, int count)
        {
            try
            {
                range = workSheet.Rows[rowIndex, this.missing];
        ...
        public void CopyRows(int rowIndex, int count)
        {
            try
            {
                range1 = workSheet.Rows[rowIndex, this.missing];
                for (int i = 1; i <= count; i++)
                {
                    range2 = workSheet.Rows[rowIndex + i, this.missing];
                    range1.Copy(range2);
                }
            }
            catch (Exception e)
        ...
        public void InsertColumns(int columnIndex, int count)
        {
            try
            {
                range = workSheet.Columns[columnIndex, this.missing];  //注意：这里和VS的智能提示不一样，第一个参数是columnindex

                for (int i = 0; i < count; i++)
                {
                    range.Insert(Microsoft.Office.Interop.Excel.XlDirection.xlDown, missing);
                    workSheet.Columns[i + 1, this.missing].Borders.LineStyle = 1;
                }
            }
            catch (Exception e)
            {
        ...
        public void DeleteColumns(int columnIndex, int count)
        {
            try
            {
                ...
                {
                    (workSheet.Cells[1, i]).EntireColumn.Delete(0);
                }
            }
            catch (Exception e)
            {
        ...
        public void SetCellComment(int rowIndex, int columnIndex, string comment)
        {
            try
            {
                dynamic range = workSheet.Cells[rowIndex, columnIndex];
        ...
        public void SaveAsFile()
        {
            ...
            try
            {
                workSheet.Cells.EntireColumn.AutoFit(); //列宽自适应
